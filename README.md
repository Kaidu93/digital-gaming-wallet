...
- **What:** Installed `i18next`, `react-i18next`, `i18next-browser-languagedetector` (all already present). Created [src/i18n/index.ts](src/i18n/index.ts) with four namespaces (`common`, `auth`, `wallet`, `bets`) and static JSON imports for both `en` and `lt` locales under [src/i18n/locales/](src/i18n/locales/). Replaced every hardcoded UI string with `t('namespace:key')` calls across all routes, features, and shared components. Added a `LanguageSwitcher` button to the header that toggles EN ↔ LT. Updated every `formatEuro` and `toLocaleString`/`toLocaleDateString` call to pass the active Intl locale via a `useLocale()` hook exported from the i18n module.
- **Why:** Bonus feature from [docs/prd.md](docs/prd.md). Static JSON imports (no lazy loading) mean the full translation bundle is always available synchronously — no `<Suspense>` wrapper needed and no flash of untranslated content.
- **How:** Language detection order is `localStorage` → `navigator`; the chosen language is stored under key `dgw-lang` so it persists across refreshes. The `useLocale()` hook maps `lt` → `lt-LT` for `Intl.NumberFormat` and `Date.toLocaleString`, falling back to `en-IE` for all other languages. Auth form validation messages (from Zod schemas) are translated via a `VALIDATION_KEYS` map inside each form component, bridging the gap between static Zod messages and runtime translations without restructuring the schema layer. Status/type badge labels use template keys (`status_win`, `type_bet`, etc.) enabling dynamic lookup via `` t(`status_${status}`) ``.

#### Task 7.3 — framer-motion animations

- **What:** Added framer-motion animations across four files — [src/components/AppShell.tsx](src/components/AppShell.tsx), [src/features/bets/components/PlaceBetForm.tsx](src/features/bets/components/PlaceBetForm.tsx), [src/routes/_authenticated/bets.tsx](src/routes/_authenticated/bets.tsx), [src/routes/_authenticated/transactions.tsx](src/routes/_authenticated/transactions.tsx), and [src/routes/_authenticated/index.tsx](src/routes/_authenticated/index.tsx).
- **Route transitions:** `AnimatePresence mode="wait"` wraps `<Outlet />` in `AppShell`. The `motion.div` child uses `key={pathname}` from `useRouterState` — this is what tells framer-motion a route change occurred; without a changing key, enter/exit never fire.
- **Balance tween:** Both the header balance (`AppShell`) and the dashboard `BalanceCard` use `useSpring` + `useTransform(spring, v => formatEuro(v, locale))` rendered via `<motion.span>`. `set(balance)` is called in a `useEffect` that also depends on `locale` so a language switch forces the spring to re-emit with the new formatter.
